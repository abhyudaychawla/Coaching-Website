        ...
        <div className="max-w-6xl mx-auto">
          <div className="max-w-3xl">
            ...
            <h1 className="font-heading text-5xl md:text-6xl text-navy leading-tight mb-4">
              Words from people who <span className="italic">found their way</span>
            </h1>
            <div className="section-divider mb-6" />
            <p className="text-base text-navy/65 leading-relaxed max-w-2xl">
              Every story here is unique. What they share in common is the courage to begin, and the willingness to stay with the process.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 px-6 border-b border-gold-light/40 bg-warm-white sticky top-18 z-30">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-wrap gap-3">
            {categories.map((cat) => (
              <span key={cat.key} className={`px-5 py-2 rounded-full text-xs tracking-wide border transition-colors ${cat.key === "all" ? "bg-navy text-warm-white border-navy" : "border-gold-light/60 text-navy/60 hover:border-gold hover:text-gold"}`}>
                {cat.label}
              </span>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <TestimonialCard key={t.id} quote={t.quote} author={t.author} location={t.location} featured={i === 0 || i === 3} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6 bg-beige">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-sm text-navy/65 leading-relaxed mb-8 max-w-lg mx-auto">
            Client privacy is taken seriously. Names and identifying details are used only with explicit consent.
          </p>
